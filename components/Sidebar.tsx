...
const Sidebar: React.FC<SidebarProps> = ({ onLogout, userEmail }) => {
  ...
  return (
    <aside className="w-[240px] bg-[#0F172A] border-r border-[#334155] flex flex-col h-screen fixed left-0 top-0 z-50 transition-colors duration-300">
      <div className="p-6">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <NavLink
                key={item.label}
                to={item.to}
                className={`flex items-center gap-3 px-4 py-3 transition-all text-sm font-medium ${
                  isActive 
      ...
      <div className="mt-auto p-6 border-t border-[#334155]">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-8 rounded-full bg-[#8B5CF6] flex items-center justify-center text-white font-bold text-xs">
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="text-xs text-[#94A3B8] truncate">{userEmail}</span>
          </div>
        </div>
        
        <button 
          onClick={onLogout}
          className="text-xs font-medium text-[#94A3B8] hover:text-[#8B5CF6] transition-all flex items-center gap-2"
        >
          <LogOut size={14} />
          <span>Log out</span>
        </button>
