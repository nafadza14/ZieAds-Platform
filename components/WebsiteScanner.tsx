...
const WebsiteScanner: React.FC<WebsiteScannerProps> = ({ onScanComplete, currentProfile }) => {
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => setProgress('Extracting High-Res Assets...'), 1000);
      setTimeout(() => setProgress('Detecting Brand DNA & Colors...'), 2000);
  ...
  const extractColorsFromImage = (src: string) => {
    ...
    img.onload = () => {
      ...
      // Simple dominant color extraction (sampling center and corners)
      const samplePoints = [
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 p-4 md:p-8 animate-in fade-in duration-700">
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-2xl shadow-slate-200/50 border border-slate-100 space-y-8">
              ...
              {isScanning && (
                <div className="space-y-6 pt-4 text-center">
                  ...
                  <p className="text-sm font-bold text-slate-400 uppercase tracking-widest animate-pulse italic">
                    {progress}
                  </p>
                </div>
              )}
              ...
              <div className="pt-6 grid grid-cols-3 gap-4">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Logo Extract</p>
                </div>
                <div className="text-center space-y-2">
                  <div className="w-12 h-12 bg-teal-50 rounded-2xl flex items-center justify-center mx-auto text-primary">
                    <Palette size={20} />
                  </div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Color Match</p>
                </div>
                <div className="text-center space-y-2">
                  <div className="w-12 h-12 bg-teal-50 rounded-2xl flex items-center justify-center mx-auto text-primary">
                    <Type size={20} />
                  </div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Voice Synth</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-6 animate-in slide-in-from-left-4 duration-500">
              ...
              <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-100 space-y-8">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Logo & Identity</label>
                    <div 
                    ...
                    >
                      {editableProfile?.logoUrl ? (
                        ...
                        <>
                          ...
                          <span className="text-[9px] text-slate-300 uppercase tracking-widest">PNG, JPG or SVG</span>
                        </>
                      )}
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Brand Colors</label>
                      {editableProfile?.logoUrl && (
                        <button 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <span className="text-[9px] font-bold text-slate-400">Primary Color</span>
                        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                          ...
                          <div className="flex-1">
                            <span className="text-[10px] font-mono font-bold uppercase block">{editableProfile?.primaryColor}</span>
                            <span className="text-[8px] text-slate-400 font-bold uppercase tracking-tight">Main CTA</span>
                          </div>
                        </div>
                      </div>
                      <div className="space-y-2">
                        <span className="text-[9px] font-bold text-slate-400">Secondary Color</span>
                        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                          <input 
                            type="color" 
                            className="w-10 h-10 rounded-lg cursor-pointer bg-transparent border-none appearance-none" 
                          ...
                          />
                          <div className="flex-1">
                            <span className="text-[10px] font-mono font-bold uppercase block">{editableProfile?.secondaryColor}</span>
                            <span className="text-[8px] text-slate-400 font-bold uppercase tracking-tight">Backgrounds</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <p className="text-[9px] text-slate-400 leading-tight">Colors are used to automatically skin your generated ad templates for instant brand consistency.</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Business Name & Summary</label>
                ...
                <button 
                  onClick={() => {
                    ...
                    setStep(3); // Proceed to Ad Accounts
                  }}
                  className="w-full py-5 tosca-bg text-white font-black text-xl rounded-2xl shadow-xl shadow-teal-500/20 flex items-center justify-center gap-3 hover:scale-[1.01] active:scale-95 transition-all"
                >
          ...
          {step === 3 && (
            <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-100 space-y-8 animate-in zoom-in-95 duration-500">
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <h3 className="text-2xl font-black text-slate-900">Final Step: Connect Channels</h3>
        ...
        {/* Right Side: Real-time Ad Preview */}
        <div className="sticky top-8 lg:block">
             ...
             <div className="bg-white rounded-[44px] overflow-hidden min-h-[600px] flex flex-col">
                <div className="h-16 border-b border-slate-100 flex items-center justify-between px-8 bg-slate-50/50">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">ZieAds Preview</span>
                   </div>
                   <div className="flex items-center gap-2">
                   ...
                   <div className="w-full max-w-[340px] space-y-8 text-center">
                      ...
                      <div className="pt-4">
                        <button 
                          ...
                          className="w-full py-4 px-8 text-white font-black text-lg rounded-2xl shadow-xl shadow-teal-500/10 transition-all hover:scale-[1.02] active:scale-95"
                        >
                          Discover More
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-6">
                        ...
                        <div className="p-3 bg-white rounded-xl border border-slate-100 shadow-sm flex items-center gap-2">
                           ...
                           <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Secondary</span>
                        </div>
                      </div>
                   </div>
                </div>
          ...
          <div className="mt-8 text-center text-[10px] font-black uppercase text-slate-400 tracking-widest flex items-center justify-center gap-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-medium text-slate-400">Marketing API v14.0</p>
          </div>
        </div>
